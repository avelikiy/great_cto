...
import { log, success, warn, error, dim } from "./ui.js";
...
export function detectLatestVersion(): string | null {
  try {
...
export function getPluginBaseDir(): string {
  return join(homedir(), ".claude", "plugins", "cache", "local", "great_cto");
}

export function findInstalledVersions(): string[] {
...
export function install(opts: { version?: string; force?: boolean } = {}): InstallResult {
  if (!hasGit()) {
    throw new Error(
      "git is required to install great_cto. Install git first: https://git-scm.com/downloads",
    );
  }
  ...
  if (supplied != null) log(dim(`  supplied ${supplied} built file(s) into packages/cli/dist/`));

  // Sanity check: can it RUN, not merely "did files arrive".
  const missing = missingRuntimeParts(pluginDir);
  if (missing.length) {
    throw new Error(
      `Install appeared to succeed but the plugin cannot run — missing:\n` +
 ...
 */
function supplyBuiltDist(pluginDir: string): number | null {
  const here = dirname(fileURLToPath(import.meta.url));   // …/dist
  const target = join(pluginDir, "packages", "cli", "dist");
  try {
    if (!existsSync(join(here, "archetypes.js"))) return null;
 ...
 */
export function missingRuntimeParts(pluginDir: string): string[] {
  const required = [
