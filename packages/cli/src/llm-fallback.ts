...
export interface LlmSuggestion {
  ...
  rationale: string;
  // Whether the user should be prompted to confirm/override the heuristic
  conflictsWithHeuristic: boolean;
}
...
export function shouldUseLlmFallback(opts: {
  ...
  forceUse: boolean;          // --use-llm flag
  forceSkip: boolean;         // --no-llm flag
}): { use: boolean; reason: string } {
  if (opts.forceSkip) return { use: false, reason: "--no-llm flag set" };
  if (process.env.GREATCTO_NO_LLM === "1") return { use: false, reason: "GREATCTO_NO_LLM=1" };
  const apiKey = process.env.ANTHROPIC_API_KEY;
  if (!apiKey) return { use: false, reason: "no ANTHROPIC_API_KEY" };
  if (opts.forceUse) return { use: true, reason: "--use-llm flag" };
  if (opts.heuristicConfidence === "low") return { use: true, reason: "low heuristic confidence" };
 ...
 */
export function buildPrompt(opts: { readme: string; stack: string[]; readmeKeywords: string[] }): string {
 ...
 */
export function parseLlmResponse(raw: string): Omit<LlmSuggestion, "conflictsWithHeuristic"> | null {
  ...
  if (typeof arch !== "string" || typeof conf !== "string" || typeof rat !== "string") return null;
  if (!ALLOWED_ARCHETYPES.includes(arch as Archetype)) return null;
  if (!["high", "medium", "low"].includes(conf)) return null;
 ...
 */
export async function suggestArchetypeFromLlm(opts: LlmCallOpts): Promise<LlmSuggestion | null> {
