...
// A hand-owned file is never touched — removing the marker line is how an author
// claims it, and the marker itself says so.
//
// But the marker also promised auto-regeneration, and this guard used to exit
// before reading the file at all, so it never happened once. This repository's
// own CODEBASE.md sat at its first-run snapshot for eleven days and twenty
// commits, asserting a file count and a set of god nodes that had moved.
//
// That is worse than the same staleness in human documentation. A person opening
// a stale ARCHITECTURE.md notices that it says thirty-four agents; an agent
// reading a stale CODEBASE.md does not notice anything — it just builds on a map
// that is wrong, silently. This is the file written FOR agents.
const CODEBASE_PATH = join(GC, 'CODEBASE.md');
const AUTOGEN_MARK = 'Generated by great_cto first-run scan';
let existingBody = null;
if (existsSync(CODEBASE_PATH)) {
  let prev = '';
  try { prev = readFileSync(CODEBASE_PATH, 'utf8'); } catch { process.exit(0); }
  if (!prev.includes(AUTOGEN_MARK)) process.exit(0);   // hand-edited; leave it alone
  // Everything after the dated header line. Comparing whole files would rewrite
  // on every run purely because the date changed, which is churn pretending to
  // be an update.
...
// Written only when the facts differ. A file rewritten on every session start
// looks like a project that changes constantly and makes its own timestamp
// meaningless; one rewritten when the map moved says exactly that.
const body = L.slice(1).join('\n').trim();
if (existingBody !== null && existingBody === body) process.exit(0);

try {
  mkdirSync(GC, { recursive: true });
  writeFileSync(CODEBASE_PATH, L.join('\n') + '\n');
} catch { /* never block */ }
