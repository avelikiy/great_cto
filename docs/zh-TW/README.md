...
**你的編碼 agent 交付程式碼。這一層負責檢查它。**

你寫一份規格。你的 Claude Code 照著它建置，另一個模型家族的第二個模型讀
同一份 diff，而在它們意見不一致的地方，你會看到那個分歧，並自己決定。
三個決定始終是你的 —— 建什麼、怎麼建、以及是否上線。最後留下的是一個
**你自己擁有的儲存庫**和一個**能用的 URL**。

公開基準測試中以這種方式建成的七個產品：token 費用**中位數 $171**，
量測於 2026-07-10。你付錢給你自己的 LLM 供應商；great_cto 是 MIT 授權，
不向你收取任何費用。
...
`localhost:3141` 的看板會自己填滿 —— **Decisions**（什麼需要你）、
**Ledger**（花了多少）、**Fleet**（該停止信任哪個代理）、**Harness**
（誰給了第二意見，以及它實際做了什麼）。看板上沒有任何東西會把「缺席」
...
加入 `PATH`；受支援的路徑是 `npx --yes great-cto@3.35.0 codex-host`：它執行受控角色、verifier、gate、recovery，以及
...
從 3.26.0 起，Codex 以第二位審查者的身分參與流水線。宣告一次即可：
...
證據是減法。v3.27.0 和 v3.27.1 刪掉了本專案自己有利的數字 ——「相對 FTE
的成本節省」、與人類團隊的開銷比較、一個推估的月份 —— 因為沒有一個能被
證明為真。
...
- **不是託管式的 app builder** —— 它不取代你的編碼 agent；沒有 agent，
  它就沒有東西可以編排。
