...
> **v2.1.0** · 30 エージェント · 22 アーキタイプ · 24 セキュリティルール · 9 フック · プロジェクトあたり ~$34/月 · 47 分で PoC · MIT
...
**言語:** [English](../../README.md) · [Русский](../ru/README.md) · [简体中文](../zh-CN/README.md) · **日本語** · [한국어](../ko/README.md) · [Español](../es/README.md)
...
- 5 つのスキャナー: prompt-injection · secrets-in-prompts · SSRF-in-tools · RAG poisoning · cost-runaway
- スタンドアロンの `@great-cto/agentshield` パッケージから統合 — 1 インストール、1 バージョン

### v1.2.0 — 継続学習ループ (2026 年 5 月)
- 新しい `continuous-learner` エージェント (Haiku, ~$0.05/実行) がセッションからパターンを自動抽出
- 二層メモリ: プロジェクトローカル `lessons.md` → クロスプロジェクト `~/.great_cto/decisions.md`
- 品質ゲート: セッションあたり最大 3 レッスン、アーキタイプタグ付け、しきい値ベースの昇格 (≥3 異なるプロジェクト)

### v1.1.0 — Claude Code フック (2026 年 5 月)
- 4 つの新フック: `secret-scan` (PreToolUse) · `format-check` (PostToolUse) · `cost-guard` (UserPromptSubmit) · `session-end`
- 13 パターンのシークレット検出カタログ (AWS, Stripe, GitHub, OpenAI, Anthropic, PEM, JWT)
- すべてのフックは `GREAT_CTO_DISABLE_<NAME>=1` でオプトアウト可能
...
|---|---|---|---|---|
| マルチエージェント SDLC パイプライン | ✓ 30 スペシャリスト | ✕ | ✕ | ✕ |
| 自動アーキタイプ検出 | ✓ 22 タイプ | ✕ | ✕ | ✕ |
