...
great_cto は **AI プロダクトビルダー**だ。ソフトウェアプロダクトを記述すれば、ビルド全体 —
アーキテクチャ、データモデル、バックエンド、フロントエンド、テスト、デプロイ — を実行する。
**人間のゲートは1つだけ:** CTO であるあなたが仕様を承認する。それ以降はすべて自動で進み、
出荷されたリポジトリとライブURLに到達する。

great_cto が構築する米国の主要産業 — ホーム＆フィールドサービス、専門サービス、
ホスピタリティ、小売 / Eコマース、プロップテック、フィットネス、マーケティング＆クリエイター、
HR / リクルーティング、建設、物流 — は、**6つの再利用可能なビルドアーキタイプ**（CRUD バーティカル SaaS、
予約、CRM、ダッシュボード、マーケットプレイス、コンテンツ / メディア）に集約される。1つのテンプレートが
...
> **Operate** — 規制対象の取引ごとに人間が署名するランタイムサーフェス（オペレーターコンソール、
> オートパイロットランタイム、バーティカルフロー） — は、**独自のリポジトリへ移動した:**
> [github.com/avelikiy/operate](https://github.com/avelikiy/operate)。great_cto は今やビルド
> プロダクトだ。
...
**推奨コンパニオンMCP: Serena（セマンティックなコードナビゲーション）。** 大規模なコードベースでは、
コードを書くエージェント（senior-dev、coder）が grep やファイル全体の読み込みでコンテキストを
消費する。[Serena MCP](https://github.com/oraios/serena) は、その代わりにシンボルレベルのナビゲーション
（find-symbol、references、structure）を与える:
...
任意 — なくてもすべて動作する。あれば、大きなリポジトリでの実装タスクが編集ごとに
明らかに少ないコンテキストで済む。
...
| 専門エージェント | **46**（architect · design-advisor · senior-dev · QA · security · devops · アーキタイプレビュアー） | 1名のジェネラリスト | 1名のジェネラリスト |
